    ...
    <section className="container py-8 md:py-10">
      <div className="flex flex-col items-center gap-12">
        ...
        <div className="grid gap-8 md:grid-cols-2">
          {customerStories.map((customStory) => (
            <div key={customStory.author.name} className="flex flex-col gap-6 rounded-lg bg-gray-500 p-6 md:p-12">
              ...
              <div className="flex items-center gap-3">
                <div className="relative h-10 w-10 overflow-hidden rounded-full">
                  <Image 
                    src={customStory.author.avatar}
                    alt={customStory.author.name}
                    fill
                    className="object-cover"
                  />
                </div>
                <div className="flex flex-col">
                  <strong className="text-gray-100 text-sm">{customStory.author.name}</strong>
                  <span className="text-xs text-gray-300">{customStory.author.role}</span>
                </div>
              </div>
            </div>
          ))}
